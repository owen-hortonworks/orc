...
public class TestOutStream {
  ...
  public void testEncryption() throws Exception {
    ...
    EncryptionAlgorithm algo = EncryptionAlgorithm.AES_128;
    byte[] keyBytes = new byte[algo.keyLength()];
  ...
  @Test
  public void testCompressionEncryption() throws Exception {
    TestInStream.OutputCollector receiver = new TestInStream.OutputCollector();
    EncryptionAlgorithm algo = EncryptionAlgorithm.AES_256;
    byte[] keyBytes = new byte[algo.keyLength()];
    for(int i=0; i < keyBytes.length; ++i) {
    ...
    InStream decompressedStream = InStream.create(name.toString(), ranges,
        compressed.length, new ZlibCodec(), 1024);
